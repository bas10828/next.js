...
const ScanBarcodePage = () => {
        ...
        switch (brand) {
          ...
          case 'UniFi':
            ...
            if (macPattern.test(first12Chars)) {
              mac = formatMacAddress(ubiquitiData.substring(0, 12));
            } else {
              mac = 'non';
            }
  ...
  const formatMacAddress = (mac) => {
    if (!mac || typeof mac !== 'string') return 'non'; // กรณีที่ mac เป็น null, undefined หรือไม่ใช่สตริง
  ...
  return (
    <Container maxWidth="full" sx={{ padding: '70px' }}>
          ...
          <Table>
            ...
            <TableBody>
              {results.map((result, index) => (
                <StyledTableRow key={`${result.fileName}-${index}`}>
...
ScanBarcodePage.displayName = 'ScanBarcodePage';
