...
function getData(project) {
  return fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/myproject/${project}`)
    .then(res => {
      if (!res.ok) {
...
export default function ProjectPage({ params }) {
  ...
  return (
    <Box sx={{ width: '100%', padding: '100px' }} className={styles['fullscreen-container']}>
      ...
      >
        โครงการ: {decodedProject}
      </Typography>
      <Box className={styles['button-container']}>
        <Link href={`/home/${project}/createproject`} passHref>
          <Button className={styles.customButton}>
            add
          </Button>
        </Link>
        <Link href={`/home/${project}/createprojectdynamic`} passHref>
          <Button className={styles.customButton}>
            Add Dynamic
          </Button>
        </Link>
            ...
            {data.map((equipment) => (
              <TableRow key={equipment.id}>
                ...
                {priority === 'user' || priority === 'admin' ? (
                  <>
                    <TableCell>
                      <Link href={`/home/${equipment.project}/update/${equipment.id}`} passHref>
