...
const Alldata = () => {
  ...
  const [isLoggedIn, setIsLoggedIn] = useState(false); // เพิ่มตัวแปร isLoggedIn
  ...
  useEffect(() => {
    ...
    if (!loggedIn) {
      setIsLoggedIn(false);
      window.location.href = "/"; // ไปยังหน้า "/" หากยังไม่ได้ล็อกอิน
    } else {
      setIsLoggedIn(true);
    }
            ...
            {data.map(equipment => (
              <TableRow key={equipment.id}>
                ...
                {priority === 'user' || priority === 'admin' ? (
                  <>
                    <TableCell>
                      <Link href={`/home/alldata/update/${equipment.id}`} passHref>
